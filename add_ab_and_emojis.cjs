...
// I need to parse the espq_q array in the generate_tests.cjs or just regenerate it with the emojis and A/B.
// The easiest way is to regenerate the espq_q array using the same base_pairs.
...
for(let i=0; i<160; i++) {
    ...
    espq_q.push({
        id: i+1,
        text: `${emoji} Pregunta ${i+1}: ¿Qué prefieres o qué te describe mejor?`,
        category: cat,
        options: [
            { label: `A) ${varA}`, value: 1 },
            { label: `B) ${varB}`, value: 2 }
        ]
    });
}
...
console.log('generate_tests.cjs updated with A/B options and emojis for ESPQ.');
